...
router.get("/:id", async (req, res) => {
    try {
      ...
      const [rows] = await db.query("SELECT * FROM marcas WHERE id = ?", [idbuscado]);
...
router.post("/", async (req, res) => {
   try {
    ...
    //insercion
    const [result] = await db.query("INSERT INTO marcas (nombremarca) VALUES (?)", [nombremarca]);
    res.status(201).json({ success: true, message: "Marca creada correctamente",id: result.insertId });
...
router.put("/:id", async (req, res) => {
   try {
    ...
    const [result] = await db.query("UPDATE marcas SET nombremarca = ? WHERE id = ?", [nombremarca, req.params.id]);
...
router.delete("/:id", async (req, res) => {
    try {
      ...
      if (productos[0].total > 0) {
        return res.status(409).json({ success: false, message: "No se puede eliminar la marca porque tiene productos asociados", products: productos[0].total });
      }
